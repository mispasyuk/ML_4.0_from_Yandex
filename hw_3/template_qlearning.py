...
def my_softmax(values: np.ndarray, T=1.):
    ...
    """
    new_values = [v-max(values) for v in values]
    exp_values = [np.exp(v/T) for v in new_values]
    exp_sum = sum(exp_values)
    probas = np.array([exp_v/exp_sum for exp_v in exp_values ])
    ...
    def get_value(self, state):
        ...
        if len(possible_actions) == 0:
          ...
          for i in range (len(possible_actions)):
            value += (probabilities[i] * qvalues[i])
